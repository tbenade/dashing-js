...
module.exports.Dashing = function Dashing() {
  ...
  dashing.ipaddress = (process.env.IPADDRESS || '0.0.0.0');
  ...
  dashing.start = function() {
    app.listen(dashing.port, dashing.ipaddress);
    logger.info('Hopefully listening on http://' + dashing.ipaddress + ':' + dashing.port + (process.env.__daemon === 'false' ? ', CTRL+C to stop' : ''));
  }
  dashing.app = app;
  return dashing;
};
